...
(function () {
  const T = {
    zh: {
      ...
      'workspace.browse_hint': '浏览器出于安全限制不返回真实路径，请手动补全完整绝对路径。所选文件夹名：{name}',
      // ── Update modal ─────────────────────────────────────────────
      'modal.update.title': '🔄 检查更新',
      'modal.update.idle': '点击"拉取更新"从 GitHub 获取最新代码',
      'modal.update.btn': '⬇ 拉取更新',
      'modal.update.restart': '⚠️ 代码已更新，请重启服务使改动生效。',
      // ── Dynamic messages ─────────────────────────────────────────
      ...
      'update.ok': '更新成功！请重启服务使改动生效。',
      'update.fail': '更新失败，请查看下方输出。',
      'update.req_fail': '请求失败：',
      'update.req_timeout': '请求超时（超过 120 秒）。可能是网络无法访问 GitHub，请检查网络连接后重试。',
    ...
    en: {
      ...
      'workspace.browse_hint': 'Browsers do not return real paths for security. Please complete the full absolute path manually. Selected folder name: {name}',
      // ── Update modal ─────────────────────────────────────────────
      ...
      'update.ok': 'Update successful! Restart the server (Ctrl+C → python app.py) to apply changes.',
      'update.fail': 'Update failed. See output below.',
      'update.req_fail': 'Request failed: ',
      'update.req_timeout': 'Request timed out (120 s). GitHub may not be reachable — check your network and try again.',
