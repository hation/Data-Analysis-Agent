// Durable Job history, event replay and browser-side sequence de-duplication (B5).
(function () {
  window.BAA = window.BAA || {};
  const vue = window.BAA.vueJobHistory;
  let sid = "";
  let lastSequence = 0;
  let pollTimer = null;
  let replaying = null;
  ...
  async function clearCompleted() {
    ...
    saveCursor();
    await refresh();
    window.BAA.overlay?.toast?.(`已清除 ${data.deleted || 0} 条任务记录`, "ok");
  }
