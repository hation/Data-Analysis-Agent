// /checkpoint — conversation-linked file history and time travel.
(function () {
  ...
  async function load() {
    if (!window.BAA.state.SID) return;
    state.loading = true; state.error = "";
    try {
      const data = await request(`/api/session/${window.BAA.state.SID}/workspace/checkpoints`);
  ...
  async function waitForJob(jobId) {
    for (let attempt = 0; attempt < 240; attempt += 1) {
      ...
      const data = await request(`/api/session/${window.BAA.state.SID}/jobs/${jobId}`);
      const job = data.job || {};
  ...
  async function refreshAfterRewind(mode) {
    if (mode !== "conversation_only") {
      await window.BAA.workspace?.loadStatus?.();
      try {
        const sourceData = await request(`/api/session/${window.BAA.state.SID}/sources`);
        window.BAA.datasource?.renderSourceList?.(sourceData.sources || []);
      } catch (_) {}
    }
  }
  ...
  async function rewind() {
    ...
    try {
      const data = await request(
        `/api/session/${window.BAA.state.SID}/workspace/checkpoints/${encodeURIComponent(target.id)}/restore`,
        {
      ...
      state.notice = "时光回退完成。";
      await load();
      window.BAA.jobHistory?.refresh?.();
      window.BAA.overlay?.toast?.("已回到所选快照", "ok");
  ...
  function openWorkspace() {
    ...
    window.BAA.workspace?.openModal?.();
  }
  ...
  window.BAA.checkpoints = { open, load };
})();
