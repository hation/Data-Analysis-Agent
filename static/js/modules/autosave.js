// Auto-save: silently saves conversation after each AI reply.
...
(function () {
  ...
  async function _restoreFromSession() {
    try {
      ...
      const { updateTokenBar } = window.BAA.msg;
      state.tokenState = {
        promptTokens:  0,
      ...
      if (updateTokenBar) updateTokenBar();
  ...
  function _showRestoreBanner(meta) {
    ...
    document.getElementById("autosave-restore-btn").addEventListener("click", () => {
      banner.remove();
      window.BAA.sessions.loadSavedSession(meta.filename, `自动保存 ${time}`);
    });
  ...
  window.BAA.autosave = { scheduleAutosave, checkAutosaveOnLoad, triggerAutosave, setLoadedName };
})();
